...
// Props
interface Props {
  autoConnect?: boolean // Следует ли автоматически обнаруживатьESP32соединять
}
...
// исследоватьxiaozhiстатус соединения
async function checkESP32Connection() {
  ...
  try {
    ...
    console.log(`${t('deviceConfig.xiaozhi')}статус соединения:`, isConnectedToESP32.value)
  }
  catch (error) {
    isConnectedToESP32.value = false
    emit('connection-status', false)
    console.log('xiaozhiПроверка соединения не удалась:', error)
  }
  finally {
...
async function scanWifi() {
  ...
  scanning.value = true
  console.log('Начало сканированияWiFiсеть')
  ...
  try {
    ...
    if (response.statusCode === 200 && response.data) {
      ...
      if (data.success && Array.isArray(data.networks)) {
        ...
        console.log(`${t('deviceConfig.scanSuccess')}，Обнаружить ${data.networks.length} ${t('deviceConfig.networks')}`)
      }
      else if (data.aps && Array.isArray(data.aps)) {
        // совместимый { data: { support_5g, aps } } Формат
        ...
        console.log(`${t('deviceConfig.scanSuccess')}，Обнаружить ${data.aps.length} ${t('deviceConfig.networks')}`)
      }
        ...
        throw new TypeError('Интерфейс сканирования возвращает исключение формата')
      }
    }
    else {
...
// Отображение селектора сети
async function showNetworkSelector() {
  // Обнаружение в реальном времениxiaozhiстатус соединения
  ...
  // Если ещё нет списка сетей，Автосканирование
...
function selectNetwork(network: WiFiNetwork) {
  ...
  console.log('Выберите сеть:', network.ssid)

  // Уведомить родительский компонент
  emit('network-selected', network, '')
}

// Уведомление родительского компонента при изменении пароля
function onPasswordChange() {
  emit('network-selected', selectedNetwork.value, password.value)
}

// Получение текущей выбранной сети и пароля
...
// Получение описания уровня сигнала
...
// Получение цвета уровня сигнала
