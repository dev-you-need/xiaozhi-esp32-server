      ...
      Api.knowledgeBase.getDocumentList(
        ...
        ({ data }) => {
          ...
          if (data && data.code === 0) {
            ...
            this.$message.error(data?.msg || 'Ошибка получения списка документов');
            this.documents = [];
            this.total = 0;
          }
        },
        (err) => {
          this.docLoading = false;
          if (err && err.data) {
            this.$message.error(err.data.msg || err.msg || 'Ошибка получения списка документов');
          } else {
            this.$message.error('Ошибка получения списка документов');
          }
          this.documents = [];
          this.total = 0;
        }
