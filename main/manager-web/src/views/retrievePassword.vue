<template>
  <div class="welcome" @keyup.enter="retrievePassword">
    <el-container style="height: 100%;">
      ...
      <el-main style="position: relative;">
        <form @submit.prevent="retrievePassword">
          <div class="login-box">
            ...
            <!-- Сохранить то же заявление о согласии -->
      ...
      <!-- Сохранить тот же нижний колонтитул -->
      <el-footer>
        <version-footer />
      </el-footer>
    </el-container>
...
export default {
  ...
  methods: {
    ...
    // Как получить многоразовые коды подтверждения
    fetchCaptcha() {
      ...
      Api.user.getCaptcha(this.form.captchaId, (res) => {
        if (res.status === 200) {
          ...
          console.error('Исключение загрузки капчи:', error);
    ...
    sendMobileCaptcha() {
      ...
      // Вызов интерфейса отправки кода подтверждения
    ...
    retrievePassword() {
      ...
      Api.user.retrievePassword({
        ...
        if (err.data != null && err.data.msg != null && (err.data.msg.indexOf('Графический код подтверждения') > -1 || err.data.msg.indexOf('Captcha') > -1)) {
          this.fetchCaptcha()
        }
      });
    },
