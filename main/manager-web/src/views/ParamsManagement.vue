...
export default {
  ...
  computed: {
    ...
    displaySections() {
      ...
      // Выберите один уровень и выберите два уровня（非 all）时，Односегментный дисплей
      ...
      // Выберите категорию первого уровня：По вторичной агрегации
      if (this.activeCategory !== 'all') {
        return this.buildSections(items, true);
      }

      // полностью：По уровню → Вторичная агрегация в виде плоских абзацев（ТО-1·ТО-2）
  ...
  watch: {
    displaySections: {
      ...
      handler(sections) {
        ...
        // Развернуть все, когда группировка короткая；Развернуть первые несколько групп по умолчанию, когда их много
  ...
  methods: {
    ...
    async fetchParams() {
      ...
      try {
        ...
        let page = 1;
        let total = Infinity;
        // 按 total Цикл Pull All Pagination，Избегайте превышения FETCH_LIMIT В агрегированном представлении отсутствуют данные, когда
        while (collected.length < total) {
          ...
          page += 1;
          // Защита：Сбой в работе total / Избегайте бесконечных циклов, когда интерфейс неисправен
          if (page > 200) break;
        }
        ...
        // Если нет данных для текущей классификации，Возврат ко всем
