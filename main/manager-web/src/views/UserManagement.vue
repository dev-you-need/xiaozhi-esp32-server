...
export default {
  ...
  methods: {
    ...
    resetPassword(row) {
      this.$confirm(this.$t('user.confirmResetPassword'), this.$t('common.warning'), {
        ...
        Api.admin.resetUserPassword(row.userid, ({ data }) => {
          if (data.code === 0) {
            // Отображение сгенерированного пароля по умолчанию
    ...
    deleteUser(row) {
      this.$confirm(this.$t('user.confirmDeleteUser'), this.$t('common.warning'), {
        ...
        Api.admin.deleteUser(row.userid, ({ data }) => {
          if (data.code === 0) {
            // Проверка необходимости корректировки номера страницы после удаления
    ...
    handleChangeStatus(row, status) {
      // Обработка одного пользователя или массива пользователей
      ...
      this.$confirm(this.$t('user.confirmStatusChange', { action: actionText, count: userCount }), this.$t('common.warning'), {
        ...
        // Действие отмены пользователем
      });
    },
    // Этот метод былbatchDeleteЗамена，Зарезервировано для обратной совместимости
    handleBatchDelete() {
      this.batchDelete();
    },
