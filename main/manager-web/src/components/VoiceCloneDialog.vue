...
export default {
    ...
    methods: {
        ...
        async handleFileChange(file) {
            ...
            // Сначала перейдите к шагу 2,убеждатьсяDOMОтражено
            this.currentStep = 2;

            // ОжиданиеDOMОбновление, затем загрузка аудио
            await this.$nextTick();
        ...
        async generateWaveform() {
            ...
            const canvas = this.$refs.waveformCanvas;
            if (!canvas) {
                console.error('CanvasЭлемент не существует');
                return;
            }

            // НастройкиcanvasРазмер
        ...
        drawWaveform() {
            const canvas = this.$refs.waveformCanvas;
            if (!canvas) {
                console.error('Отрисовка формы волны Canvas id не существует.');
                return;
            }
            ...
            if (this.waveformData.length === 0) {
                console.error('Данные кривой пусты');
                return;
            }

            // Найдите максимальное значение для нормализации
            ...
            this.waveformData.forEach((value, index) => {
                // Нормализация и увеличение，Эксплуатация 80%Высота
        ...
        async handleTrim() {
            ...
            // Повторная генерация формы волны
        ...
        handleTimeUpdate() {
            // Прогресс воспроизведения можно обновить здесь
        },
        handleAudioEnded() {
            this.isPlaying = false;
        },
        async bufferToFile(buffer) {
            // -AudioBufferСтоимостьWAVДокументация:  
        ...
        async uploadAudio() {
            ...
            // Проверить продолжительность звука（8-60второй）
            if (this.audioBuffer) {
                const duration = this.audioBuffer.duration;
