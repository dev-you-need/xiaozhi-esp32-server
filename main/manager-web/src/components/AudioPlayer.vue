...
// Возврат пути иконки в зависимости от состояния громкости
...
const toggleMute = () => {
  ...
  // Если снять с mute и громкость равна 0, восстановить громкость по умолчанию
  if (!isMuted.value && volume.value === 0) {
    volume.value = 0.5
...
// Функция перетаскивания полосы прогресса
...
// Управление видимостью ползунка громкости
...
// Позиционирование ползунка громкости
const updateSliderPosition = () => {
  nextTick(() => {
    ...
    // Вычисление позиции относительно окна просмотра
    const left = buttonRect.left + window.scrollX
    const top = buttonRect.top + window.scrollY

    // Позиционирование точно по центру кнопки, вертикальное центрирование
