...
export default {
  ...
  watch: {
    visible(val) {
      ...
      if (val) {
        // Загрузить список моделей RAG при отображении диалога
        this.loadRAGModels();

        // Если добавляется база знаний и не настроенаragModelId，则默认选择第一个RAG模型
    ...
    // Отслеживание изменений списка моделей RAG, Обеспечение корректной установки значений по умолчанию при добавлении
    ragModels(newModels) {
      if (newModels.length > 0) {
        // Если добавляется база знаний и не настроена ragModelId, то по умолчанию выбирается первая модель RAG
  ...
  methods: {
    handleClose() {
      // Не сбрасывать поля формы, чтобы при редактировании сохранять предыдущий выбор
      // Сбрасывать состояние проверки только при закрытии диалога
      if (this.$refs.knowledgeBaseForm) {
        this.$refs.knowledgeBaseForm.clearValidate();
      }
    ...
    loadRAGModels() {
      if (this.ragModels.length > 0) {
        return; // Уже загружено, Избежание повторной загрузки
      }

      console.log('Начало загрузкиRAG模型列表');
