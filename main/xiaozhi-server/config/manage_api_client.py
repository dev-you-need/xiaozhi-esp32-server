...
class ManageApiClient:
    _instance = None
    _async_clients = {}  # Хранить отдельные клиенты для каждого цикла событий
    _secret = None
    ...
    def _init_client(cls, config):
        ...
        cls.max_retries = cls.config.get("max_retries", 6)  # Максимальное количество попыток
        cls.retry_delay = cls.config.get("retry_delay", 10)  # Первоначальная задержка повтора (секунды)
        # Не создавайте здесь AsyncClient, отложите до фактического использования
        cls._async_clients = {}
    ...
    async def _ensure_async_client(cls):
        ...
        try:
            ...
            # Создание отдельных клиентов для каждого цикла событий
            if loop_id not in cls._async_clients:
                # Сервер может проактивно закрыть соединение, и пул соединений httpx не может быть правильно обнаружен и очищен
                limits = httpx.Limits(
                    max_keepalive_connections=0,  # Отключите keep-alive, создавайте новое соединение каждый раз
                )
                cls._async_clients[loop_id] = httpx.AsyncClient(
                    ...
                    limits=limits,  # Объем эксплуатационных ограничений
                    trust_env=False,
                )
            return cls._async_clients[loop_id]
        except RuntimeError:
            # Если нет запущенного цикла событий, создайте временный
            raise Exception("必须在异步上下文中调用")

    @classmethod
    async def _async_request(cls, method: str, endpoint: str, **kwargs) -> Dict:
        """发送单次异步HTTP请求并处理响应"""
        # Убедитесь, что клиент создан
        ...
        try:
            ...
            # Бизнес-ошибка при обработке API
            ...
            # Возврат данных об успехе
            return result.get("data") if result.get("code") == 0 else None
        finally:
            # Убедитесь, что ответ закрыт (даже если выполняется исключение)
    ...
    async def _execute_async_request(cls, method: str, endpoint: str, **kwargs) -> Dict:
        ...
        while retry_count <= cls.max_retries:
            try:
                ...
                # Определите, следует ли повторить попытку
                if retry_count < cls.max_retries and cls._should_retry(e):
                    ...
                    # Бросьте исключение без повторной попытки
                    raise
