...
/**
 * Код ошибки, состоит из 5 цифр, первые 2 — код модуля, последние 3 — код бизнеса
 * <p>
 * Например: 10001 (10 — модуль системы, 001 — бизнес-код)
 * </p>
...
public interface ErrorCode {
    ...
    // Коды ошибок, связанные с проверкой параметров
    ...
    // Ошибка удаления модели по умолчанию
    int DEFAULT_MODEL_DELETE_ERROR = 10064;
    // Коды ошибок, связанные с входом в систему
    int ADD_DATA_FAILED = 10065; // 新增数据Ошибка
    int UPDATE_DATA_FAILED = 10066; // 修改数据Ошибка
    int SMS_CAPTCHA_ERROR = 10067; // 短信验证码错误
    int MOBILE_REGISTER_DISABLED = 10068; // 未开启手机注册
    int USERNAME_NOT_PHONE = 10069; // 用户名不是номер телефона码
    int PHONE_ALREADY_REGISTERED = 10070; // номер телефона码已注册
    int PHONE_NOT_REGISTERED = 10071; // номер телефона码未注册
    int USER_REGISTER_DISABLED = 10072; // 不允许用户注册
    int RETRIEVE_PASSWORD_DISABLED = 10073; // 未开启找回密码功能
    int PHONE_FORMAT_ERROR = 10074; // номер телефона码формат不正确
    int SMS_CODE_ERROR = 10075; // 手机验证码错误

    // Коды ошибок, связанные с типами словарей
    int DICT_TYPE_NOT_EXIST = 10076; // Тип словаря不存在
    int DICT_TYPE_DUPLICATE = 10077; // Тип словаря编码重复

    // Коды ошибок, связанные с обработкой ресурсов
    int RESOURCE_READ_ERROR = 10078; // 读取资源Ошибка

    // Коды ошибок, связанные с интеллектуальными агентами
    int LLM_INTENT_PARAMS_MISMATCH = 10079; // LLM大模型和Intent意图识别，选择参数不匹配

    // Коды ошибок, связанные с голосовым отпечатком
    ...
    int VOICEPRINT_REGISTER_REQUEST_ERROR = 10087; // 声纹保存请求Ошибка
    int VOICEPRINT_REGISTER_PROCESS_ERROR = 10088; // 声纹保存处理Ошибка
    int VOICEPRINT_UNREGISTER_REQUEST_ERROR = 10089; // 声纹注销请求Ошибка
    int VOICEPRINT_UNREGISTER_PROCESS_ERROR = 10090; // 声纹注销处理Ошибка
    int VOICEPRINT_IDENTIFY_REQUEST_ERROR = 10091; // 声纹识别请求Ошибка
    ...
    // Коды ошибок управления сервером
    ...
    // Коды ошибок проверки параметров
    int WEBSOCKET_URLS_EMPTY = 10098; // WebSocket地址列表不能为空
    int WEBSOCKET_URL_LOCALHOST = 10099; // WebSocket地址不能使用localhost或127.0.0.1
    int WEBSOCKET_URL_FORMAT_ERROR = 10100; // WebSocket地址формат不正确
    int WEBSOCKET_CONNECTION_FAILED = 10101; // WebSocket连接测试Ошибка
    ...
    int OTA_INTERFACE_ACCESS_FAILED = 10106; // OTA接口访问Ошибка
    int OTA_INTERFACE_FORMAT_ERROR = 10107; // OTA接口返回内容формат不正确
    int OTA_INTERFACE_VALIDATION_FAILED = 10108; // OTA接口验证Ошибка
    ...
    int MCP_INTERFACE_ACCESS_FAILED = 10112; // MCP接口访问Ошибка
    int MCP_INTERFACE_FORMAT_ERROR = 10113; // MCP接口返回内容формат不正确
    int MCP_INTERFACE_VALIDATION_FAILED = 10114; // MCP接口验证Ошибка
    ...
    int VOICEPRINT_INTERFACE_ACCESS_FAILED = 10119; // 声纹接口访问Ошибка
    int VOICEPRINT_INTERFACE_FORMAT_ERROR = 10120; // 声纹接口返回内容формат不正确
    int VOICEPRINT_INTERFACE_VALIDATION_FAILED = 10121; // 声纹接口验证Ошибка
    int MQTT_SECRET_EMPTY = 10122; // Ключ MQTT不能为空
    int MQTT_SECRET_LENGTH_INSECURE = 10123; // Ключ MQTT长度不安全
    int MQTT_SECRET_CHARACTER_INSECURE = 10124; // Ключ MQTT必须同时包含大小写字母
    int MQTT_SECRET_WEAK_PASSWORD = 10125; // Ключ MQTT包含弱密码
    int DICT_LABEL_DUPLICATE = 10128; // 字典标签重复
    int SM2_KEY_NOT_CONFIGURED = 10129; // SM2密钥未配置
    int SM2_DECRYPT_ERROR = 10130; // SM2解密Ошибка
    int MODEL_TYPE_PROVIDE_CODE_NOT_NULL = 10131; // modelType和provideCode不能为空

    // Коды ошибок, связанные с историей чата
    ...
    int CHAT_HISTORY_DOWNLOAD_FAILED = 10135; // 聊天记录下载Ошибка
    ...
    // Коды ошибок, связанные с клонированием голоса
    ...
    int VOICE_CLONE_UPLOAD_FAILED = 10143; // 上传Ошибка
    ...
    int VOICE_CLONE_TRAINING_FAILED = 10154; // Ошибка обучения
    int VOICE_CLONE_HUOSHAN_CONFIG_MISSING = 10155; // 火山引擎缺少配置
    int VOICE_CLONE_RESPONSE_FORMAT_ERROR = 10156; // 响应формат错误
    int VOICE_CLONE_REQUEST_FAILED = 10157; // 请求Ошибка
    int VOICE_CLONE_PREFIX = 10158; // 复刻音色前缀
    int VOICE_ID_ALREADY_EXISTS = 10159; // 音色ID已存在
    int VOICE_CLONE_HUOSHAN_VOICE_ID_ERROR = 10160; // 火山引擎音色IDформат错误

    // Коды ошибок, связанные с устройствами
    int MAC_ADDRESS_ALREADY_EXISTS = 10161; // Mac地址已存在
    // Коды ошибок, связанные с моделями
    int MODEL_PROVIDER_NOT_EXIST = 10162; // 供应器不存在

    // Коды ошибок, связанные с базой знаний
    int Knowledge_Base_RECORD_NOT_EXISTS = 10163; // 知识库记录不存在
    int RAG_CONFIG_NOT_FOUND = 10164; // RAG配置未找到
    int RAG_CONFIG_TYPE_ERROR = 10165; // Тип конфигурации RAG错误
    int RAG_DEFAULT_CONFIG_NOT_FOUND = 10166; // 默认RAG配置未找到
    int RAG_API_ERROR = 10167; // RAG调用Ошибка
    int UPLOAD_FILE_ERROR = 10168; // 上传文件Ошибка
    ...
    int RAG_API_ERROR_URL_INVALID = 10174; // RAG配置中base_urlформат不正确，请检查协议是否正确
    ...
    // Коды ошибок устройств (дополнение)
    int MCA_NOT_NULL = 10175; // mac地址不能为空

    // Клонирование голоса (дополнение)
    int VOICE_CLONE_NAME_NOT_NULL = 10181; // 音色克隆名称不能为空
    int VOICE_CLONE_AUDIO_NOT_FOUND = 10182; // 音色克隆音频不存在

    // Коды ошибок шаблонов агентов (дополнение)
    int AGENT_TEMPLATE_NOT_FOUND = 10183; // 默认智能体未找到

    // Коды ошибок адаптеров базы знаний
    int RAG_ADAPTER_TYPE_NOT_SUPPORTED = 10184; // 不支持的适配器类型
    int RAG_CONFIG_VALIDATION_FAILED = 10185; // RAG配置验证Ошибка
    int RAG_ADAPTER_CREATION_FAILED = 10186; // 适配器创建Ошибка
    int RAG_ADAPTER_INIT_FAILED = 10187; // 适配器初始化Ошибка
    int RAG_ADAPTER_CONNECTION_FAILED = 10188; // 适配器连接测试Ошибка
    int RAG_ADAPTER_OPERATION_FAILED = 10189; // 适配器操作Ошибка
    ...
    // Коды ошибок инструментов устройств
    int DEVICE_ID_NOT_NULL = 10193; // 设备ID不能为空
    int DEVICE_NOT_EXIST = 10194; // 设备不存在
    int OTA_UPLOAD_COUNT_EXCEED = 10195; // Количество загрузок OTA超过限制

    // Коды ошибок меток агентов
