...
public class VoiceCloneServiceImpl extends BaseServiceImpl<VoiceCloneDao, VoiceCloneEntity>
    ...
    public void save(VoiceCloneDTO dto) {
        ...
        // Проверка, используется ли Voice ID
        ...
        // Перебор выбранных идентификаторов тембров, создание записи для каждого идентификатора тембра
        int index = 0;
        ...
        for (String voiceId : dto.getVoiceIds()) {
            ...
            entity.setTrainStatus(0); // По умолчанию обучение
    ...
    public PageData<VoiceCloneResponseDTO> pageWithNames(Map<String, Object> params) {
        // Сначала запросить данные пагинации
        IPage<VoiceCloneEntity> page = baseDao.selectPage(
                getPage(params, "create_date", true),
                getWrapper(params));
    ...
    public VoiceCloneResponseDTO getByIdWithNames(String id) {
        ...
        // Убедиться, что поле trainStatus установлено корректно, фронтенду нужно это поле для определения, является ли аудио клонированным
    ...
    /**
     * Преобразование списка VoiceCloneEntity в список VoiceCloneResponseDTO
     */
    private List<VoiceCloneResponseDTO> convertToResponseDTOList(List<VoiceCloneEntity> entityList) {
        ...
        // Получить набор идентификаторов пользователей
        ...
        // Преобразование каждой сущности в DTO
        for (VoiceCloneEntity entity : entityList) {
            ...
            // Установить, есть ли аудиоданные
    ...
    public void uploadVoice(String id, MultipartFile voiceFile) throws Exception {
        // Запрос записи клонирования голоса
        VoiceCloneEntity entity = baseDao.selectById(id);
        if (entity == null) {
            throw new RenException(ErrorCode.VOICE_CLONE_RECORD_NOT_EXIST);
        }

        // Чтение аудиофайла и преобразование в массив байтов
        byte[] voiceData = voiceFile.getBytes();

        // Обновление поля voice
        entity.setVoice(voiceData);
        // Обновление статуса обучения на ожидающее обучение
    ...
    /**
     * Вызов движка Volcano для обучения голосового клонирования
     * 
     * @param config Конфигурация модели
     * @param entity Сущность записи клонирования голоса
     * @throws Exception
     */
    private void huoshanClone(Map<String, Object> config, VoiceCloneEntity entity) throws Exception {
        ...
        if (baseResp != null) {
            ...
            if (statusCode != null && statusCode == 0 && StringUtils.isNotBlank(speakerId)) {
                ...
                // При неудаче использовать StatusMessage как сообщение об ошибке
